...
def random_generator(batch_size, z_dim, T_mb, max_seq_len):
    ...
    """
    Z_mb = []
    actual_batch_size = min(batch_size, len(T_mb))
    ...
    # If T_mb is shorter than batch_size, pad Z_mb with random sequences
    while len(Z_mb) < batch_size:
        temp = np.zeros([max_seq_len, z_dim])
        temp_Z = np.random.uniform(0.0, 1, [max_seq_len, z_dim])
        temp[:, :] = temp_Z
        Z_mb.append(temp)
